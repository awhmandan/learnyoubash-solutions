...
characters almost in any script. So lets discover the secret what it  
exactly is.  
 
### Streams  
 
Bash receives input and sends output as sequences or streams of  
characters. These streams may be redirected into files or one into  
another.  
...
if the first positional parameter is false, path to current directory if  
second parameter is true. If third parameter is true output the list of  
files in current directory or Third parameter is false. if it is false.  
...
command to another. Lists of commands give you opportunity to change the  
result of executing of your script.  
 
You are already familiar with ls command. But what if you need to list all  
files with specific extension in current directory?  
